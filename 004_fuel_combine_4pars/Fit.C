...
int main()
{
  ...
  int par_index = 0;
  min.SetVariable( par_index, "sigma_239", 4, 1e-4 ); par_index++;
  min.SetVariable( par_index, "sigma_235", 6, 1e-4 ); par_index++;
  min.SetVariable( par_index, "sigma_238", 11, 1e-4 ); par_index++;
  min.SetVariable( par_index, "sigma_241", 8, 1e-4 ); par_index++;
  ...
  for( int idx=0; idx<4; idx++ ) {
    ofstream out_VOC_best("out_VOC_best.dat", ios::out | ios::app);
    out_VOC_best<<par_val[idx]<<"\t"<<par_err[idx]<<endl;
    out_VOC_best.close();
  }
  ...
  double fit_VOC_s239_1s[Npoints] = {0};
  double fit_VOC_s235_1s[Npoints] = {0};

  double fit_VOC_s239_2s[Npoints] = {0};
  double fit_VOC_s235_2s[Npoints] = {0};

  double fit_VOC_s239_3s[Npoints] = {0};
  double fit_VOC_s235_3s[Npoints] = {0};

  min.SetErrorDef( 2.2977 ); min.Contour(0,1, Npoints_usr, fit_VOC_s239_1s, fit_VOC_s235_1s);
  min.SetErrorDef( 6.2021 ); min.Contour(0,1, Npoints_usr, fit_VOC_s239_2s, fit_VOC_s235_2s);
  min.SetErrorDef(11.6182 ); min.Contour(0,1, Npoints_usr, fit_VOC_s239_3s, fit_VOC_s235_3s);

  ofstream out_VOC_1s("out_VOC_1s.dat", ios::out | ios::app);
  ofstream out_VOC_2s("out_VOC_2s.dat", ios::out | ios::app);
  ofstream out_VOC_3s("out_VOC_3s.dat", ios::out | ios::app);
  for(int idx=0; idx<Npoints; idx++) {
    out_VOC_1s<<fit_VOC_s239_1s[idx]<<"\t"<<fit_VOC_s235_1s[idx]<<endl;
    out_VOC_2s<<fit_VOC_s239_2s[idx]<<"\t"<<fit_VOC_s235_2s[idx]<<endl;
    out_VOC_3s<<fit_VOC_s239_3s[idx]<<"\t"<<fit_VOC_s235_3s[idx]<<endl;
  }
...
double fcn(const double *par)
{
  ...
  for( int idx=0; idx<iso_N; idx++ ) {
    sigmaf_DYB[idx][0] = sigmaf_RCT[idx][0];
  }
